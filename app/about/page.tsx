  ...
  return (
    <>
      ...
      <div className="yye-pattern relative bg-yye-dark px-[5%] pt-[140px] pb-20 overflow-hidden">
        {/* Decorations */}
        <div className="absolute top-[-80px] right-[-80px] w-[500px] h-[500px] bg-yye-green opacity-[0.14] rounded-full pointer-events-none blur-3xl" style={{ zIndex: 1 }} />
        <div className="absolute bottom-[-60px] right-[220px] w-[280px] h-[280px] bg-yye-yellow opacity-[0.10] rounded-full pointer-events-none blur-2xl" style={{ zIndex: 1 }} />

        <div className="relative z-[2] max-w-[680px]">
          <p className="text-[12px] font-semibold text-white/40 tracking-[0.08em] uppercase mb-4">
            <Link href="/" className="text-yye-yellow no-underline hover:underline">Home</Link>
            {' '}/ About
          </p>
          <h1
          ...
          >
            We are{' '}
            <em className="text-yye-yellow not-italic">more than</em>{' '}
            an organization.
          </h1>
      ...
      <section id="story" className="bg-yye-light px-[5%] py-24">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-20 items-center">
          ...
          <RevealWrapper delay={80}>
            ...
            <h2 className="section-title">Culturally grounded. Globally ready.</h2>
            <p className="text-base text-yye-gray leading-[1.7] mb-6">
              We believe knowing where you come from gives you the strength to go anywhere. YYE adopts a community-driven and impact-focused approach by working closely with schools, local leaders, and partners to design programs that address real challenges faced by young people.
            </p>
      ...
      <section className="yye-pattern bg-yye-dark px-[5%] py-24">
        ...
        <RevealWrapper delay={120}>
          <div className="mt-12">
            <p className="text-[1.1rem] font-bold text-white/45 mb-6 tracking-[0.05em]">Core Objectives</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {objectives.map((obj) => (
                <div key={obj} className="bg-white/[0.04] border border-white/[0.08] rounded-[12px] px-[1.4rem] py-5 flex items-start gap-3">
                  <div className="w-2 h-2 min-w-[8px] bg-yye-yellow rounded-full mt-[6px]" />
                  <p className="text-[13px] text-white/75 leading-[1.6]">{obj}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {approach.map(({ num, title, desc }, i) => (
            <RevealWrapper key={num} delay={i * 80}>
              <div className="bg-yye-light border border-yye-green/[0.1] rounded-[16px] p-[1.8rem]">
                <div className="text-[2rem] font-extrabold text-yye-green opacity-30 mb-2">{num}</div>
